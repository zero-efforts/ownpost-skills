...
For writing, read get_assistant_context once to reuse personal preferences and approved voice. For performance analysis, read get_performance_context. Reuse fresh context during the task; refresh revisions immediately before edits. Read unrelated history only when the task needs it. Parallelize independent reads when the client supports it; keep dependent writes in order.
